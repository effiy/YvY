# 原始URL: https://comfyui-wiki.com/zh/tutorial/basic/how-to-inpaint-an-image-in-comfyui

# 抓取时间: 2025-03-30 21:02:05

[Skip to content](https://comfyui-wiki.com/zh/tutorial/basic/how-to-inpaint-an-image-in-comfyui#reach-skip-nav)
...
[![RunComfy](https://comfyui-wiki.com/patron/runcomfy-logo.png)RunComfyComfyUI Cloud](https://www.runcomfy.com)[![Comfy Deploy](https://comfyui-wiki.com/patron/comfydeploy-logo.png)Comfy DeployRun and deploy your ComfyUI workflows](https://comfydeploy.com/?utm_source='comfyui-wiki'&utm_content='sidebar')[![Comfy Online](https://comfyui-wiki.com/patron/comfyonline-logo.png)Comfy OnlineRun ComfyUI workflows online and deploy APIs with one click](https://www.comfyonline.app/)[![Comfy.ICU](https://comfyui-wiki.com/patron/comfy-icu-logo.png)Comfy.ICURun ComfyUI in the cloud](https://comfy.icu/)[![InstaSD](https://comfyui-wiki.com/patron/instasd-logo.png)InstaSDHow creative teams run & deploy workflows](https://www.instasd.com/)
  * [开始局部重绘工作流前的准备（可选）](https://comfyui-wiki.com/zh/tutorial/basic/how-to-inpaint-an-image-in-comfyui#开始局部重绘工作流前的准备可选)
  * [局部重绘工作流素材](https://comfyui-wiki.com/zh/tutorial/basic/how-to-inpaint-an-image-in-comfyui#局部重绘工作流素材)
  * [局部重绘工作流讲解](https://comfyui-wiki.com/zh/tutorial/basic/how-to-inpaint-an-image-in-comfyui#局部重绘工作流讲解)


[](https://forms.gle/LRjjgeMwDYLdwXbk9)
[系列教程](https://comfyui-wiki.com/zh/tutorial "系列教程")[ComfyUI 基础入门教程](https://comfyui-wiki.com/zh/tutorial/basic "ComfyUI 基础入门教程")7.局部重绘
←→
...
这些都可以通过 **图像局部重绘** 来实现，但本篇是一个较为基础的局部重绘工作流，在[ComfyUI 官方局部重绘工作流示例](https://comfyui-wiki.com/zh/workflows/inpaint) 你可以找到官方使用[VAE内补编码器](https://comfyui-wiki.com/zh/comfyui-nodes/latent/inpaint/vae-encode-for-inpaint)进行局部重绘的示例。 但这本篇教程里，我将使用[设置Latent噪波遮罩](https://comfyui-wiki.com/zh/comfyui-nodes/latent/inpaint/set-latent-noise-mask)来完成局部重绘的示例。
本篇教程主要示例是修改图像中人物的眼睛颜色和表情。
![局部重绘效果](https://comfyui-wiki.com/_next/static/media/inpainting.d9c2902e.jpg)
## 开始局部重绘工作流前的准备（可选）[](https://comfyui-wiki.com/zh/tutorial/basic/how-to-inpaint-an-image-in-comfyui#开始局部重绘工作流前的准备可选)
这个工作流将会使用到以下模型，但不是必须的，除非你的电脑上没有对应SD1.5模型:
模型类型| 模型名称| 下载链接| 说明  
---|---|---|---  
SD1.5 模型| DreamShaper v8| [前往下载](https://civitai.com/models/4384?modelVersionId=128713)| SD1.5模型的基础模型  
下载后请将这个模型文件保存在以下目录:
```
📁ComfyUI
├── 📁models
│  ├── 📁checkpoints
│  │  └── 📁SD1.5                 // 新建一个 SD1.5 文件夹
│  │    └── dreamshaper_8.safetensors      // 存放DreamShaper v8
```

## 局部重绘工作流素材[](https://comfyui-wiki.com/zh/tutorial/basic/how-to-inpaint-an-image-in-comfyui#局部重绘工作流素材)
请下载下面的图片和并点击按钮下载工作流文件
![局部重绘输入图片](https://comfyui-wiki.com/_next/static/media/inpanit-input.efc8e1a3.png)
ComfyUI 局部重绘工作流
## 局部重绘工作流讲解[](https://comfyui-wiki.com/zh/tutorial/basic/how-to-inpaint-an-image-in-comfyui#局部重绘工作流讲解)
打开 ComfyUI 后将工作流文件拖入 ComfyUI 中，或者使用菜单打开工作流文件来加载 ![局部重绘工作流示例](https://comfyui-wiki.com/_next/static/media/inpainti-workflow.fdbd121c.jpg)
  1. 请在序号`1`处加载`DreamShaper v8 `模型，或者那你电脑上的其它类型的模型
  2. 请在序号`2`处[Load Image](https://comfyui-wiki.com/zh/comfyui-nodes/image/load-image)加载在上一个步骤提供的图片


在 绘制需要重绘的部分的蒙版，具体完整的绘制模板的方法请参考[遮罩编辑器](https://comfyui-wiki.com/zh/interface/maskeditor)的使用方法。
![遮罩编辑器](https://comfyui-wiki.com/_next/static/media/open-in-maskeditor.47005fbb.jpg)
  1. 修改序号`3`处 [CLIP Text Encode](https://comfyui-wiki.com/zh/comfyui-nodes/conditioning/clip-text-encode)的文本，在我的工作流中是`smile, black eyes,close mouth` 你可以修改为你想要的画面内容
  2. 使用 Queue 或者快捷键 `Ctrl(Command)+Enter` 运行工作流进行图片生成，可能需要经历几次重新生成才会有满意的效果
...
```
  ...
  1. 试着调整不同的`denoise`参数，查看不同的生成效果,先从一个比较小的值比如 0.1 开始
...
![局部重绘效果](https://comfyui-wiki.com/_next/static/media/inpainting.d9c2902e.jpg)
![局部重绘输入图片](https://comfyui-wiki.com/_next/static/media/inpanit-input.efc8e1a3.png)
![局部重绘工作流示例](https://comfyui-wiki.com/_next/static/media/inpainti-workflow.fdbd121c.jpg)
