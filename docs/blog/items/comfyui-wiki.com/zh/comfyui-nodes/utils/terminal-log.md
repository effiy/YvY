# 原始URL: https://comfyui-wiki.com/zh/comfyui-nodes/utils/terminal-log

# 抓取时间: 2025-03-30 21:03:18

[Skip to content](https://comfyui-wiki.com/zh/comfyui-nodes/utils/terminal-log#reach-skip-nav)
...
[节点详解](https://comfyui-wiki.com/zh/comfyui-nodes "节点详解")[Utils](https://comfyui-wiki.com/zh/comfyui-nodes/utils/note "Utils")Terminal Log (Manager)
←→
[新闻|字节跳动发布 InfiniteYou：保留用户身份特征的灵活图像再创作框架2025/03/21](https://comfyui-wiki.com/zh/news/2025-03-21-bytedance-infiniteyou)
# Terminal Log(Mangager) 终端日志(管理器)
![ComfyUI Node Menu - Terminal LogNode](https://comfyui-wiki.com/utils/Terminal_Log.jpg)
Terminal Log 这个节点主要用于在 ComfyUI 界面展示 ComfyUI 的在CMD终端的运行信息，如需使用，需要将`mode模式` 设置为 **logging** 模式，则可以在图片生成任务进行时记录对应的日志信息，如`mode`设置为**stop** 模式，则不会记录日志信息。 当你是采用远程连接或者局域网连接的方式来访问和使用 ComfyUI 时，这个节点的作用就很明显，它让你可以直接在ComfyUI的界面中就可以直接看到 CMD 中的报错信息，更方便你了解当前 ComfyUI 的运行状态。
Last updated on 2025年3月24日
[Reroute 转接点](https://comfyui-wiki.com/zh/comfyui-nodes/utils/reroute "Reroute 转接点")[ComfyUI 官方示例](https://comfyui-wiki.com/zh/workflows "ComfyUI 官方示例")
中文
System
