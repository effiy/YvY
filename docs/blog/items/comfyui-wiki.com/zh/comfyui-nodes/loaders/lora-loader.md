# 原始URL: https://comfyui-wiki.com/zh/comfyui-nodes/loaders/lora-loader

# 抓取时间: 2025-03-30 21:03:04

[Skip to content](https://comfyui-wiki.com/zh/comfyui-nodes/loaders/lora-loader#reach-skip-nav)
...
[![RunComfy](https://comfyui-wiki.com/patron/runcomfy-logo.png)RunComfyComfyUI Cloud](https://www.runcomfy.com)[![Comfy Deploy](https://comfyui-wiki.com/patron/comfydeploy-logo.png)Comfy DeployRun and deploy your ComfyUI workflows](https://comfydeploy.com/?utm_source='comfyui-wiki'&utm_content='sidebar')[![Comfy Online](https://comfyui-wiki.com/patron/comfyonline-logo.png)Comfy OnlineRun ComfyUI workflows online and deploy APIs with one click](https://www.comfyonline.app/)[![Comfy.ICU](https://comfyui-wiki.com/patron/comfy-icu-logo.png)Comfy.ICURun ComfyUI in the cloud](https://comfy.icu/)[![InstaSD](https://comfyui-wiki.com/patron/instasd-logo.png)InstaSDHow creative teams run & deploy workflows](https://www.instasd.com/)
  * [输入类型](https://comfyui-wiki.com/zh/comfyui-nodes/loaders/lora-loader#输入类型)
  * [输出类型](https://comfyui-wiki.com/zh/comfyui-nodes/loaders/lora-loader#输出类型)
  * [LoRA 额外补充](https://comfyui-wiki.com/zh/comfyui-nodes/loaders/lora-loader#lora-额外补充)
  * [例子：](https://comfyui-wiki.com/zh/comfyui-nodes/loaders/lora-loader#例子)
  * [使用具体使用示例及工作流](https://comfyui-wiki.com/zh/comfyui-nodes/loaders/lora-loader#使用具体使用示例及工作流)


[](https://forms.gle/LRjjgeMwDYLdwXbk9)
[节点详解](https://comfyui-wiki.com/zh/comfyui-nodes "节点详解")[Loaders](https://comfyui-wiki.com/zh/comfyui-nodes/loaders/checkpoint-loader-simple "Loaders")Lora Loader - ComfyUI 节点说明
←→
[新闻|字节跳动发布 InfiniteYou：保留用户身份特征的灵活图像再创作框架2025/03/21](https://comfyui-wiki.com/zh/news/2025-03-21-bytedance-infiniteyou)
# Load LoRA - LoRA 加载器
![LoRA加载器节点](https://comfyui-wiki.com/_next/static/media/Load_LoRA.57b3d715.jpg)
这个节点会自动检测位于 LoRA 文件夹下的模型(包括子文件夹)对应模型路径为 `ComfyUI\models\loras`，更多请参考 [安装 LoRA 模型](https://comfyui-wiki.com/zh/install/install-models/install-lora)
...
如果需要加载 多个LoRA 模型，你可以直接将多个节点进行一个串联, 如下图所示 ![加载多个模型](https://comfyui-wiki.com/_next/static/media/Load_LoRA-2.42e277fb.jpg)
## 输入类型[](https://comfyui-wiki.com/zh/comfyui-nodes/loaders/lora-loader#输入类型)
参数名称| 数据类型| 作用  
---|---|---  
...
## LoRA 额外补充[](https://comfyui-wiki.com/zh/comfyui-nodes/loaders/lora-loader#lora-额外补充)
  1. **LoRA的灵活性：**
     * LoRA通过对已有模型进行微调，可以迅速生成特定风格的图像，而无需重新训练整个模型。它是“画家的工具”，让你能更精确地控制画面效果。
  2. **资源效率：**
     * 使用LoRA模型的好处是占用的计算资源较少，避免了大规模模型训练的时间成本。它只需对目标效果进行微调，既高效又节省资源。
  3. **多样化的应用：**
     * LoRA不仅仅能改变人物形象，也可以调整背景、色调、光影、天气等元素，为作品增添丰富的细节。
  4. **可定制性与兼容性：**
     * LoRA模型支持个性化定制，可以根据需求创建自定义的LoRA模型，并且可以组合使用多个LoRA模型，实现更多复杂的效果。
  5. **适应不同领域：**
     * LoRA不仅限于艺术风格和人物特征，它还可以用来调节情绪氛围、改变光照环境、增加特定物体细节等。


### 例子：[](https://comfyui-wiki.com/zh/comfyui-nodes/loaders/lora-loader#例子)
  * **水墨画风格** ：LoRA模型能够调整色彩渲染和笔触，模拟水墨画的层次感。
  * **游戏角色特征** ：LoRA能够调整角色的衣着、发型等细节，打造游戏角色的风格。


## 使用具体使用示例及工作流[](https://comfyui-wiki.com/zh/comfyui-nodes/loaders/lora-loader#使用具体使用示例及工作流)
[SD1.5 基础 LoRA 工作流](https://comfyui-wiki.com/zh/tutorial/basic/lora) [ComfyUI 基础 LoRA 工作流](https://comfyui-wiki.com/zh/workflows/lora) [ComfyUI 官方原始工作流](https://comfyanonymous.github.io/ComfyUI_examples/lora/)
Last updated on 2025年3月24日
[Hypernetwork Loader](https://comfyui-wiki.com/zh/comfyui-nodes/loaders/hypernetwork-loader "Hypernetwork Loader")[Lora Loader Model Only](https://comfyui-wiki.com/zh/comfyui-nodes/loaders/lora-loader-model-only "Lora Loader Model Only")
中文
System
